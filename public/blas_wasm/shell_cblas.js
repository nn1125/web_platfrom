...
function findWasmBinary() {
  return locateFile("shell_cblas.wasm");
}
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(3, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  offset = bigintToI53Checked(offset);
  return -52;
}

function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 0, 1, addr, len, prot, flags, fd, offset);
  offset = bigintToI53Checked(offset);
}
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, __mmap_js, __munmap_js, _environ_get, _environ_sizes_get, _fd_close, _fd_seek, _fd_write ];

// Imports from the Wasm binary.
var _run_command, _malloc, _free, _main, __emscripten_tls_init, _pthread_self, _emscripten_builtin_memalign, __emscripten_thread_init, __emscripten_thread_crashed, __emscripten_run_js_on_main_thread_done, __emscripten_run_js_on_main_thread, __emscripten_thread_free_data, __emscripten_thread_exit, __emscripten_check_mailbox, _emscripten_stack_set_limits, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, __indirect_function_table, wasmTable;

function assignWasmExports(wasmExports) {
  _run_command = Module["_run_command"] = wasmExports["run_command"];
