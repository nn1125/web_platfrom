  ...
  async solve(ctx) {
    ...
    if (potrfRows.length === n) {
      ...
      if (solution) {
        ...
        const verifyMat = parseMat(posvOut);
        const verifySol = verifyMat.length > 0 ? verifyMat.flat() : null;
        if (verifySol) {
          let match = true;
          for (let i = 0; i < n; i++) if (Math.abs(solution[i] - verifySol[i]) > 1e-4) { match = false; break; }
          solHtml += `<div class="verify ${match ? 'verify--ok' : 'verify--fail'}">Проверка через LAPACKE_dposv: ${match ? 'совпадает' : 'расхождение!'}</div>`;
        }

        s = stepLog.addStep('Решение системы', null, solHtml, potrsCmd);
        await stepLog.showStep(s);
